...
func ExecuteHealthCheck(ctx context.Context, task containerd.Task, container containerd.Container, hc *Healthcheck) error {
	...
	processSpec, err := prepareProcessSpec(hc, container, ctx)
	if err != nil {
		return err
	}
	if processSpec == nil {
...
func probeHealthCheck(ctx context.Context, task containerd.Task, hc *Healthcheck, processSpec *specs.Process) (*HealthcheckResult, error) {
	...
	// TODO: Use a limited buffer instead of bytes.Buffer when capturing health check output.
	// Reason: bytes.Buffer grows unbounded and could consume significant memory if a health check command produces excessive output.
	// See Docker's limitedBuffer implementation for reference. (we do this during inspect)
	// TODO Limiting output size to 4096 bytes (with possible truncation indication) protects against accidental or maliciously verbose commands.
...
func prepareProcessSpec(hcConfig *Healthcheck, container containerd.Container, ctx context.Context) (*specs.Process, error) {
