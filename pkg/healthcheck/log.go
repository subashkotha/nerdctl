...
// Consider keeping health state in memory and only syncing periodically, or caching recent state and truncating logs in memory before writing to disk.
...
func ReadHealthLogForInspect(stateDir string) (*Health, error) {
	...
	for _, logEntry := range h.Log {
		if len(logEntry.Output) > MaxOutputLen {
			lb := &limitedBuffer{}
...
func (b *limitedBuffer) Write(p []byte) (int, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	remaining := MaxOutputLen - b.buf.Len()
	if remaining <= 0 {
		b.truncated = true
		return len(p), nil
	}
